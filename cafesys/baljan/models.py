...
class ShiftCombination(Made):
    ...
    def __str__(self):
        return "%s: %s (%s)" % (
            self.label,
            ', '.join([str(sh)
                      for sh in self.shifts.all().order_by('when', 'span')]),
            self.semester,
        )
...
class Shift(Located):
    ...
    exam_period = models.BooleanField(_("exam period"),
                                      help_text=_(
                                          'the work scheduler takes this field into account'),
                                      default=False)
...
class ShiftSignup(Made):
    ...
    class Meta:
        ...
        permissions = (
            ('self_and_friend_signup', _nl(
                "Can sign up self and friends")),  # for workers
        )
...
class Good(Made):
    ...
    description = models.CharField(
        _("short description"), blank=True, max_length=100)
...
class Order(Located):
    put_at = models.DateTimeField(
        _("put at"), default=datetime.now, db_index=True)
...
class RefillSeries(Made):
    ...
    add_to_group = models.ForeignKey("auth.Group", verbose_name=_("add to group"),
                                     help_text=_(
                                         "if set, users will be added to this group"),
    ...
    def is_present(cls, user, policy_name, minor=1, major=None):
        if major is None:
            query = LegalConsent.objects.filter(
                user=user, policy_name=policy_name, policy_version__gte=minor, revoked=False)
        else:
            query = LegalConsent.objects.filter(
                user=user, policy_name=policy_name, policy_version__gte=minor, policy_version__lte=major, revoked=False)
