...
class Made(models.Model):
    made = models.DateTimeField(_("made at"), help_text=_(
        "when the object was created"), auto_now_add=True)
...
class Profile(Made):
    ...
    show_profile = models.BooleanField(
        "Visa mitt namn i topplistan", default=True)
    ...
    def has_free_blipp(self):
        return self.user.has_perm("baljan.free_coffee_unlimited") or self.user.has_perm(
            "baljan.free_coffee_with_cooldown")
    ...
    class Meta:
        ...
        permissions = (
            ("available_for_call_duty", _nl(
                "Available for call duty")),  # for workers
...
def traderequest_notice_delete(tr):
    if tr.answered:
        ...
        if cancel:
            logger.info(
                "not sending message about trade request deletion because the offered or wanted shift was in the past"
            )
            return
...
class Semester(Made):
    ...
    def clean(self):
        ...
        if not (self.name[:2] in ("HT", "VT")
                and len(self.name) == len("HT2010")):
...
# objects caused by the Meta.ordering below. Solved by:
# semester.shiftcombination_set.order_by()
class ShiftCombination(Made):
    ...
    def is_free(self):
        ...
        return len([sh for sh in self.shifts.all()
                   if sh.signups().count() != 0]) == 0
...
class Shift(Located):
    ...
    def accepts_signups(self):
        return (
            self.upcoming()
            and self.semester.signup_possible
            and self.signups().count() < 2
            and self.span != 1
        )
...
def signup_notice_save(signup):
    ...
    # If we are in a transaction-less context, this function will be called
    # immediately.
...
class Good(Made):
    ...
    def cost(self, day):
        try:
            gc = self.goodcost_set.filter(
                from_date__lt=day).order_by("-from_date")[0]
            return gc
        except IndexError:
            return None
...
class RefillSeries(Made):
    ...
    code_currency = models.CharField(
        _("code currency"), max_length=5, default="SEK")
...
class BalanceCode(Made):
    ...
    value = models.PositiveIntegerField(
        _("value"), default=BALANCE_CODE_DEFAULT_VALUE)
...
class OldCoffeeCard(models.Model):
    set = models.ForeignKey(
        OldCoffeeCardSet, verbose_name=_("set"), on_delete=models.CASCADE
    )
...
class BlippConfiguration(Located):
    ...
    card_reader_short_endianess = models.CharField(
        ...
        help_text=(
            '"Byte order" för korta RFID-nummer (fyra bytes). '
