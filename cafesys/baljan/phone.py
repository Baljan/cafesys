...
def _get_current_worker_phone_numbers(location=Located.KARALLEN):
    ...
    workers = ShiftSignup.objects.filter(
        shift__span=current_span, shift__when=date.today(), shift__location=location
    ).select_related("user__profile")
...
def compile_incoming_call_response(request):
    ...
    if ivr_key is not None:
        if ivr_key == "failed":
            ...
            user_has_permission = not required_permission or (
                required_permission
                and request.from_user
                and request.from_user.has_perm(required_permission)
            )
