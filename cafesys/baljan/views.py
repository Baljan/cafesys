...
def home(request):
    ...
    next_shifts_baljan = models.Shift.objects.filter(
        when__gte=today, location=0, enabled=True
    ).order_by("when", "span")
    next_shifts_byttan = models.Shift.objects.filter(
        when__gte=today, location=1, enabled=True
    ).order_by("when", "span")
    next_day_shifts_baljan = models.Shift.objects.filter(
        when=Subquery(next_shifts_baljan.values("when")[:1])
    )
    next_day_shifts_byttan = models.Shift.objects.filter(
        when=Subquery(next_shifts_byttan.values("when")[:1])
    )
    next_day_baljan = (
        next_day_shifts_baljan[0].when if len(next_day_shifts_baljan) else None
    )
    next_day_byttan = (
        next_day_shifts_byttan[0].when if len(next_day_shifts_byttan) else None
    )
...
class CafeWeekView(WeekArchiveView):
    ...
    def get_queryset(self):
        return models.Shift.objects.filter(
            location=self.location, enabled=True
        ).order_by("when")
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.add_message(
                request,
                messages.SUCCESS,
                "Tack för din beställning! Ni kommer få en bekräftelse när styrelsen har behandlat din beställning.",
            )
...
def staff_homepage(request):
    today = date.today()
    upcoming_shifts = models.ShiftSignup.objects.filter(
        user=request.user, shift__when__gte=today
    ).order_by("shift__when", "shift__span")
    upcoming_callduty = models.OnCallDuty.objects.filter(
        user=request.user, shift__when__gte=today
    ).order_by("shift__when", "shift__span")
...
def semester(request, name=None, loc=0):
    selectable_semesters = models.Semester.objects.visible_to_user(
        request.user
    ).order_by("-start")
    ...
    if name is None:
        ...
        sem = get_object_or_404(
            models.Semester.objects.visible_to_user(request.user), name__exact=name
        )
    ...
    if sem:
        tpl["shifts"] = (
            models.Shift.objects.order_by("when", "span")
            .filter(semester_id=sem.id, enabled=True, location=loc)
...
def day_shifts(request, day):
    ...
    tpl = {}
    tpl["semester"] = models.Semester.objects.visible_to_user(request.user).for_date(
        day
    )
    tpl["day"] = day
    tpl["shifts"] = models.Shift.objects.filter(when=day, enabled=True).order_by(
        "location", "span"
    )
    tpl["available_for_call_duty"] = avail_call_duty = available_for_call_duty()

    if request.method == "POST":
        assert request.user.is_authenticated and request.user.has_perm(
            "baljan.add_oncallduty"
        )
        ...
        assert signup_user not in [
            callduty.user for callduty in shift.oncallduty_set.all()
        ]
...
def card_id(request, signed_rfid=None):
    ...
    if request.method == "GET" and signed_rfid is not None:
        try:
            ...
            tpl["signature_error"] = (
                "Registreringslänken du använder har gått ut, påbörja registreringen på nytt."
            )
        except (BadSignature, ValueError):
            tpl["signature_error"] = (
                "Registreringslänken du använder är felaktig, påbörja registreringen på nytt."
            )

    if request.method == "POST":
        form = forms.ProfileCardIdForm(request.POST, instance=user.profile)
        if form.is_valid():
            form.save()
...
def see_group(request, group_name):
    ...
    tpl["other_groups"] = (
        pseudogroups.real_only().exclude(name__exact=group_name).order_by("name")
    )
...
def trade_take(request, signup_pk, redir):
    ...
    try:
        ...
        messages.add_message(
            request, messages.ERROR, _("This would result in a double booking.")
        )
        return redirect_prepend_root(redir)
...
def call_duty_week(request, year=None, week=None):
    ...
    if request.method == "POST" and is_ajax(request):
        ...
        # add new users
        for error in models.OnCallDuty.bulk_add_shifts(
...
def do_blipp(request):
    ...
    if user is None:
        ...
        return _json_error(
            404,
            "Blippkortet är inte kopplat till någon användare",
            signed_rfid=signed_rfid,
        )
    ...
    if has_cooldown:
        try:
            latest_order = models.Order.objects.filter(accepted=True, user=user).latest(
                "put_at"
            )
            order_cooldown_date = latest_order.put_at + timedelta(
    ...
    if "authorization" in request.headers:
        ...
        if len(authorization) == 2:
            if authorization[0].lower() == "token":
                ...
                try:
                    return BlippConfiguration.objects.select_related("good").get(
                        token=token
                    )
...
def stats_order_heatmap(request):
    ...
    orders = (
        ...
        .filter(weekday__lt=6, hour__gte=8, hour__lte=16)
        .values("weekday", "hour", "quarter")
        .annotate(count=Count("id"))
        .order_by("weekday", "hour", "quarter")
    )
    ...
    for timepoint in orders:
        data.at[
            weekdays[timepoint["weekday"] - 1],
            time(timepoint["hour"], timepoint["quarter"]),
        ] = int(timepoint["count"])
...
def stats_blipp(request):
    ...
    balance_codes = (
        models.BalanceCode.objects.filter(
            used_at__isnull=False, used_at__year__gte=from_year
        )
        .values("used_at")
    ...
    orders = (
        models.Order.objects.filter(
            accepted=True, paid__gt=0, put_at__year__gte=from_year
        )
        .extra({"day": "date(put_at)"})
        .values("day")
        .annotate(count=Sum("paid"))
        .order_by("day")
    )
    ...
    plot = sns.relplot(
        data={
            "Kaffekort använda": bc_data.loc[:, "count"],
            "Blippat för": order_data.loc[:, "count"],
        },
        kind="line",
    )
    plot.figure.autofmt_xdate()
...
def stats_active_blipp_users(request):
    ...
    orders = (
        f.qs.filter(accepted=True)
        .annotate(week=F("put_at__week"), year=F("put_at__year"))
        .order_by("year", "week")
        .values("week", "year")
