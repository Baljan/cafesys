...
def send(notification_type, to_user, wait=False, **kwargs):
    ...
    if wait: 
        send_mail_task(title, body, f'Baljan <{settings.DEFAULT_FROM_EMAIL}>', [to_user.email])
    else: 
        send_mail_task.delay(title, body, f'Baljan <{settings.DEFAULT_FROM_EMAIL}>', [to_user.email])
