...
def categories_and_actions(request):
    ...
    all_links = [('superusers', 'Superanvändare', (
    ...
    all_pages = [('superusers', 'Superanvändare', (
        ...
        (settings.WORKER_GROUP, 'Jobbare', (
            Action('Personer och grupper', 'search_person'), # TODO: make permissions worker only
            Action('Jobbplanering', 'current_semester'),
            )),
        ('regulars', 'Ditt konto', tuple(regulars_upcoming_sem_actions)),
        ('anyone', 'Användare', (
            Action('Info', 'staff_homepage'),
        )),]
